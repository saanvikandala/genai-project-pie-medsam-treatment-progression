...
def main(args):
    ...
    #i changed this to rename the cache (in case it overwrites the finetuned checkpoints cache)
    pipe = StableDiffusionPIEPipeline.from_pretrained(model_id_or_path, torch_dtype=torch.float32, cache_dir="./hf_cache", safety_checker=None)
    #pipe = StableDiffusionPIEPipeline.from_pretrained(model_id_or_path, torch_dtype=torch.float32, safety_checker=None)
    
    if finetuned_path != None:
        #i changed this to take in a unet instead of the text_encoder
